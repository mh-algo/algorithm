...
class Trie:
	...
	def search(self,string):
		...
		if curr_node.children:        # 다음 노드가 존재할 경우(문자열이 접두어인 경우)
			return False
		else:       # 다음 노드가 존재하지 않을 경우(문자열이 접두어가 아닌 경우)
			return True
...
print(trie.search('app'))      # app이 접두어인 단어가 존재하므로 False 출력
print(trie.search('app'))      # app이 접두어인 단어가 존재하지 않으므로 True 출력
